...
describe('ta', function () {
    ...
    it('指标简单计算 - 全部默认参数', function () {
        ...
        assert.equal(ind1.outs.ma1(-1), 9999);
        assert.equal(ind1.outs.ma2(-1), 9995.5); // N2 默认参数 10
        assert.equal(ind1.outs.ma3(-1), 9985.5); // N2 默认参数 30
        assert.equal(ind1.outs.ma4(-1), 9970.5); // N2 默认参数 60
    });
    ...
    it('指标简单计算 - 部分默认参数', function () {
        ...
        assert.equal(ind1.outs.ma2(-1), 9995.5);
    });
    ...
    it('指标简单计算 - 客户端填入全部参数', function () {
        ...
        assert.equal(ind1.outs.ma1(-1), 10000);
        assert.equal(ind1.outs.ma2(-1), 9999);
        assert.equal(ind1.outs.ma3(-1), 9998);
        assert.equal(ind1.outs.ma4(-1), 9997);
    });
    ...
    it('更新指标计算范围', function () {
        ...
        let outs_data2 = ind.outs.ma2(1000, 1010);
        ...
        let outs_data3 = ind.outs.ma2(9990, 10000);

        assert.equal(outs_data3.length, 11);
        assert.equal(outs_data3[0], 9989);
        assert.equal(outs_data3[4], 9993);
        assert.equal(outs_data3[10], 9999);
    ...
    it('指标简单计算 - 取没有 klines 数据的范围', function () {
        ...
        let outs_data = ind.outs.ma1(100, 110);
        ...
        let outs_data2 = ind.outs.ma1(10000, 10010);
        assert.equal(outs_data2.length, 11);
        assert.equal(outs_data2[0], 9999);
        assert.ok(isNaN(outs_data2[1]));
