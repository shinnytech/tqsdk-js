...
function* demo(C){
    ...
    // 输出序列
    // 计算
    C.TRADE_AT_CLOSE(true);
    // TRADE_OC_CYCLE
    while(true) {
        let i = yield;
        console.log(C.DS.open[i] % 2)
...
describe('指标中下单', function () {
    ...
    it('常规流程', function () {
        ...
        assert.equal(send_obj.datas.mk[0].length, 201);


        // batch_input_datas({TQ, symbol, dur:5, left_id:1000, right_id:3001, last_id:3001});
