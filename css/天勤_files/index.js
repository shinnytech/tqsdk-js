...
$(function () {
    ...
    $('#btn_editor_run').on('click', function (e) {
        ...
        if (CMenu.editing.type === 'custom_wh') {
            ...
            covertWH(CMenu.editing, code).then(function(){
                Notify.success('转换成功');
            },function(){
                Notify.error('转换失败');
            });
...
function covertWH(indicator, code) {
    let type = 'MAIN';
    ...
    for (let i = 1; i <= 6; i++) {
        ...
        if (p.name)
            params.push([p.name, Number(p.default_value), Number(p.min), Number(p.max)]);
    }
    var req = {
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            if (response.errline > -1) {
                Notify.error('convert/wh  line:' + response.errline + ' col:' + response.errcol + '\n' + response.errvalue);
                reject();
            } else {
                worker.postMessage({cmd: 'indicator', content: {name: indicator.name, code: response.target}});
                resolve();
            }
        }, function (e) {
            console.error(e);
