...
while(true){
let i = yield;
LC[i]=REF(i, C.DS.close, 1);
AA[i]=ABS((C.DS.high[i] - LC[i]));
BB[i]=ABS((C.DS.low[i] - LC[i]));
CC[i]=ABS((C.DS.high[i] - REF(i, C.DS.low, 1)));
DD[i]=ABS((LC[i] - REF(i, C.DS.open, 1)));
R[i]=(((AA[i] > BB[i]) && (AA[i] > CC[i])) ? ((AA[i] + (BB[i] / 2)) + (DD[i] / 4)) : (((BB[i] > CC[i]) && (BB[i] > AA[i])) ? ((BB[i] + (AA[i] / 2)) + (DD[i] / 4)) : (CC[i] + (DD[i] / 4))));
X[i]=(((C.DS.close[i] - LC[i]) + ((C.DS.close[i] - C.DS.open[i]) / 2)) + LC[i]) - REF(i, C.DS.open, 1);
